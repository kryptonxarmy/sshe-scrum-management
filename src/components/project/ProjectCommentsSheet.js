  ...
  return (
    <div className="border-b mx-4 pb-4 mb-4">
      ...
      <div className="flex gap-2 mt-2">
        <Button variant="ghost" className={`p-2`} size="xs" onClick={() => onReply(id)}>
          Reply
        </Button>
      </div>
      {replyingId === id && (
        <form className="flex flex-col gap-2 mt-2" onSubmit={(e) => onSubmitReply(e, id)}>
          ...
          <div className="flex gap-2">
            <Button type="submit" className={`p-2`} size="xs" disabled={!replyContent.trim()}>
              Kirim Reply
            </Button>
            <Button type="button" className={`p-2`} size="xs" variant="outline" onClick={() => onReply(null)}>
              Batal
            </Button>
          </div>
        </form>
...
export default function ProjectCommentsSheet({ open, onOpenChange, projectId, user }) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="max-w-lg w-full">
        <SheetHeader>
          <SheetTitle>Komentar Project</SheetTitle>
          <SheetDescription>Diskusi dan update terkait project ini.</SheetDescription>
        </SheetHeader>
