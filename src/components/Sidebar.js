...
const Sidebar = ({ currentFunction, setCurrentFunction, isSidebarOpen, setIsSidebarOpen }) => {
  ...
  // Popup state for feature coming soon
  const [popup, setPopup] = useState({ open: false, message: "" });

  const showTeam = () => {
    setPopup({ open: true, message: "Team management feature coming soon!" });
  };

  const showCalendar = () => {
    setPopup({ open: true, message: "Calendar feature coming soon!" });
  };

  const showSettings = () => {
    setPopup({ open: true, message: "Settings feature coming soon!" });
  };

  const showProfile = () => {
    setPopup({ open: true, message: "Profile feature coming soon!" });
  };
    ...
    <>
      ...
      <Dialog open={popup.open} onOpenChange={(open) => setPopup((prev) => ({ ...prev, open }))}>
        <DialogContent className="max-w-sm">
          ...
          <div className="text-blue-600">{popup.message}</div>
          <DialogFooter>
            <Button onClick={() => setPopup((prev) => ({ ...prev, open: false }))}>OK</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <aside
